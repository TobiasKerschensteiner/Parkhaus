...
def func_impressum():
    impressum_mouse_position = pygame.mouse.get_pos()

    screen.fill(0, 0, 0)

    font_cap = pygame.font.Font(None, 75)

    text_impressum_menu = font_cap.render("Impressum", True, color_text)
    impressum_menu = text_impressum_menu.get_rect(center=(150, 150))
    screen.blit(text_impressum_menu, impressum_menu)

    button_impressum_menu = Button(image=None, pos=(200, 300),
                                   text_input="BACK", font=pygame.font.Font(None, 75), base_color="White",
                                   hovering_color="Green")

    button_impressum_menu.changeColor(impressum_mouse_position)
    button_impressum_menu.update(screen)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if button_impressum.collidepoint(impressum_mouse_position):
                main()
